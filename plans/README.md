# Implementation Plans

Generated by the improve skill on 2026-06-20 and extended on 2026-06-21. Execute
in the order below unless dependencies say otherwise. Each executor should read
the plan fully before starting, honor its STOP conditions, and update the
relevant row when done.

## Execution Order And Status

| Plan | Title | Priority | Effort | Depends on | Status |
|---|---|---|---|---|---|
| 001 | Guard history timestamp conversion | P1 | S | - | DONE |
| 002 | Surface validation warnings and agnix failures | P1 | M | - | DONE |
| 003 | Cover the fallback write and mirror path | P1 | M | - | DONE |
| 004 | Implement real CLI help output | P2 | S | - | DONE |
| 005 | Surface prompt-dump diagnostics | P2 | S | - | DONE |
| 006 | Show interactive warning diagnostics | P2 | S | - | DONE |
| 007 | Offer extra history source when defaults exist | P2 | M | - | DONE |
| 008 | Restore invalid primary skill writes | P1 | M | - | DONE |
| 009 | Continue history discovery after source errors | P1 | S | - | DONE |
| 010 | Install runtime signal handlers once | P2 | S | - | DONE |
| 011 | Enforce selected skill name validation | P2 | S | - | DONE |
| 012 | Cover release notes extraction | P3 | S | - | DONE |
| 013 | Filter injected context at the final prompt boundary | P1 | S | - | DONE |
| 014 | Bound history discovery and parsing work | P1 | M | 013 | DONE |
| 015 | Make the CLI stdin lifecycle idempotent | P2 | S | - | DONE |
| 016 | Resolve the agent discovery contract in the docs | P2 | S | - | DONE |
| 017 | Spike a bounded local history index or cache strategy | P3 | M | 013, 014 | DONE |
| 018 | Tighten existing-skill duplicate scoring | P2 | M | - | DONE |

Status values: TODO | IN PROGRESS | DONE | BLOCKED (with one-line reason) |
REJECTED (with one-line rationale).

## Dependency Notes

- Plans 001, 002, 004, and 005 are independent bug/DX fixes.
- Plan 003 is a test-coverage plan and should land before any larger refactor
  of the interactive write flow.
- If Plan 002 lands before Plan 003, Plan 003 should assert validation warnings
  only if they naturally appear in the tested fallback path. Do not add generic
  warning assertions just to couple the two plans.
- Plan 006 complements Plan 005. Plan 005 covers diagnostics for the `prompts`
  command; Plan 006 covers diagnostics in the default interactive flow.
- Plan 007 is independent. If it lands before Plan 006, make sure any new
  integration tests still account for warning output changes later.
- Plans 008 and 011 both touch validation-related behavior but are independent:
  Plan 008 handles cleanup/restore after validation failure, while Plan 011
  tightens the validation rule for selected skill names.
- Plan 009 should land before any broader refactor of history discovery because
  it defines the partial-failure behavior expected from source traversal.
- Plan 010 is independent and should remove the `MaxListenersExceededWarning`
  noise observed during the current full Vitest run.
- Plan 012 is independent test coverage for release-note extraction.
- Plan 013 is a privacy/correctness guard for prompt extraction and should land
  before broader scan/index work.
- Plan 014 depends on Plan 013 so large-history work does not preserve or
  amplify leaked injected context.
- Plan 015 is independent and closes the remaining runtime listener lifecycle
  gap after Plan 010.
- Plan 016 is a docs/design alignment plan and should land before any future
  implementation of structured agent-discovery result parsing.
- Plan 017 depends on Plans 013 and 014 because a cache/index strategy should be
  designed only after prompt filtering and bounded scans are in place.
- Plan 018 is independent. It should land before any broader changes to fallback
  ranking because it affects which locally ranked candidates users can see.

## Verification Baseline From Recon

- Current post-plan audit was run at commit `0925fb7`.
- The 2026-06-21 extension plans were written at commit `d5148d0`.
- No validation commands were run by the advisor during the 2026-06-21
  extension pass; only read/search commands and plan-file writes were performed.
- `bun run check` passed at audit time.
- `bun run typecheck` passed at audit time.
- `bun run test` passed at audit time: 8 files, 66 tests. The run emitted
  `MaxListenersExceededWarning` for `SIGINT` and `SIGTERM`; Plan 010 covers this
  DX issue.
- `bun audit` reported no vulnerabilities.
- `bun run build` and `bun run verify` were not run by the advisor because build
  writes `dist/`; executors may run them when their operator allows build
  artifacts.

## Findings Considered And Rejected

- Privileged local agent flags in `src/skills/draft.ts`: not filed as a
  security bug because `docs/TECH_SPEC.md` explicitly documents launching Claude
  with `--dangerously-skip-permissions` and Codex with `--yolo` as part of the
  chosen local-agent handoff model.
- Release workflow permissions in `.github/workflows/release.yml`: not filed as
  a security bug because the workflow requests `contents: write` only, which
  matches the release-note/GitHub Release requirement in `docs/TECH_SPEC.md`.
- Dependency vulnerability sweep: not filed because `bun audit` reported no
  vulnerabilities.
- Agent discovery structured-result parsing: identified as a product direction
  option, not an implementation bug. It was not planned in this batch because
  the request was to turn the five vetted findings into plans; structured
  discovery parsing should be handled as a separate design/spike if selected.
